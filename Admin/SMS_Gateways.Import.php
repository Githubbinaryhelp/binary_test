...
<body>
  <center>
	  <h1>Read the contents of CSV File</h1>
			...
			<?
			$row = 1;
				if (($handle = fopen("SMS_Gateways.Import.txt", "r")) !== FALSE) {
				    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
				        $num = count($data);
				        echo "<p> $num fields in line $row: <br /></p>\n";
				        $row++;
				        //for ($c=0; $c < $num; $c++) {
				            echo "Reading Line: ". $data[0] . "," .$data[1] . ",". $data[2]. "<br>";
				            $ColumnValue1=htmlspecialchars($data[0]);
							$ColumnValue2=htmlspecialchars($data[1]);
							$ColumnValue3=htmlspecialchars($data[2]);

						$sql="INSERT INTO SMS_Gateways (GatewayName, SMSGateway, MMSGateway) 
